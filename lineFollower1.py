...
def on_press(key):
    ...
    if frozenset(current_keys) in combination_to_function:
        ...
        # Ustawianie promienia skrętu:
            ...
            if mo_cap is False and autoEnv :
                # song.start()
                auto = True
                autospeed = 70
...
trackbars = np.zeros((1, 600, 3), np.uint8)
cv2.namedWindow('Trackbars')
if mo_cap:
    cv2.createTrackbar('Kp', 'Trackbars', 50, 100, empty)
    cv2.createTrackbar('Kd', 'Trackbars', 32, 100, empty)
    cv2.createTrackbar('H_min', 'Trackbars', 40, 255, empty)
    cv2.createTrackbar('S_min', 'Trackbars', 150, 255, empty)
    cv2.createTrackbar('V_min', 'Trackbars', 70, 255, empty)
    cv2.createTrackbar('H_max', 'Trackbars', 95, 255, empty)
...
while True:
    ...
    if motion_capture:
        ...
        if auto:
            ...
            clientsocket.send(bytes(msgmaker.msgmaker1(total_error, speed) + "\n", "utf-8"))
    ...
    # imgStacked = utlis.stackImages(1, ([result, map], [image, mask]))
    # imgStacked = utlis.stackImages(1, ([result, map], [image, mask]))
    # Nagrywanko:
